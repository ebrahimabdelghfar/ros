...
void CMNodeHelloCM::odomFillMsg(nav_msgs::msg::Odometry& msg) {
  ...
  // Ground truth position from CarMaker (global frame)
  // Using Fr1 (Frame 1 = vehicle body frame) position in world coordinates
  msg.pose.pose.position.x = Car.Fr1.t_0[0];  // X position in world frame
  msg.pose.pose.position.y = Car.Fr1.t_0[1];  // Y position in world frame
  msg.pose.pose.position.z = Car.Fr1.t_0[2];  // Z position in world frame
  
  // Ground truth orientation from CarMaker 
  // Using Fr1 frame rotation angles
  tf2::Quaternion q;
  q.setRPY(Car.Fr1.r_zyx[0], Car.Fr1.r_zyx[1], Car.Fr1.r_zyx[2]);
  msg.pose.pose.orientation = tf2::toMsg(q);
  
  // Ground truth linear velocity in world frame from Fr1
  msg.twist.twist.linear.x = Vehicle.v;  // Velocity X in world frame
  msg.twist.twist.linear.y = Car.Fr1.v_0[1];  // Velocity Y in world frame
  msg.twist.twist.linear.z = Car.Fr1.v_0[2];  // Velocity Z in world frame
  // Ground truth angular velocity from Vehicle struct
  msg.twist.twist.angular.x = Vehicle.RollVel;   // Roll rate (rad/s)
  msg.twist.twist.angular.y = Vehicle.PitchVel;  // Pitch rate (rad/s)
  msg.twist.twist.angular.z = Vehicle.YawRate;   // Yaw rate (rad/s)
}

void CMNodeHelloCM::tireSteeringFillMsg(hellocm_msgs::msg::TireSteering& msg) {
  // Header with simulation time
  msg.header.stamp = rclcpp::Time(static_cast<int64_t>(1e9 * SimCore.Time), RCL_ROS_TIME);
  msg.header.frame_id = "base_link";
  
  // Individual wheel steering angles from Car.Susp structure
  // Car.Susp[0].SteerAngle = Front Left tire steer angle [ISO 8855 4.1.5.1] [rad]
  // Car.Susp[1].SteerAngle = Front Right tire steer angle [ISO 8855 4.1.5.1] [rad]
  // Indices: 0=FL, 1=FR, 2=RL, 3=RR
  msg.front_left_angle = Car.Susp[0].SteerAngle;
  msg.front_right_angle = Car.Susp[1].SteerAngle;
  
  // Mean front tire steering angle (average of left and right)
  msg.tire_angle = (Car.Susp[0].SteerAngle + Car.Susp[1].SteerAngle) / 2.0;
  
  // Reference pinion angle from VehicleControl for comparison
  msg.pinion_angle = VehicleControl.Steering.Ang;
}

void CMNodeHelloCM::tireForcesFillMsg(hellocm_msgs::msg::TireForcesArray& msg) {
  // Header with simulation time
  msg.header.stamp = rclcpp::Time(static_cast<int64_t>(1e9 * SimCore.Time), RCL_ROS_TIME);
  msg.header.frame_id = "base_link";
  
  // Helper lambda to fill individual tire data
  auto fillTireData = [](hellocm_msgs::msg::TireForces& tire_msg, int tire_idx) {
    // Get tire handle - accessing global Tire array from CarMaker
    tTire* tire = VhclModel_Tire_GetHandle(tire_idx);
    
    if (tire != nullptr) {
      // Tire identification
      tire_msg.tire_index = static_cast<uint8_t>(tire_idx);
      
      // Contact Point Forces in wheel coordinate system (FrW) [N]
      tire_msg.fx = tire->IF.Frc_W[0];  // Longitudinal force
      tire_msg.fy = tire->IF.Frc_W[1];  // Lateral force
      tire_msg.fz = tire->IF.Frc_W[2];  // Normal force (vertical load)
      
      // Contact Point Torques in wheel coordinate system (FrW) [Nm]
      tire_msg.mx = tire->IF.Trq_W[0];  // Overturning moment
      tire_msg.my = tire->IF.Trq_W[1];  // Rolling resistance moment
      tire_msg.mz = tire->IF.Trq_W[2];  // Self-aligning torque (Mz)
      
      // Tire Slip Parameters (Pacejka Model Inputs)
      tire_msg.slip_angle = tire->IF.Alpha;      // Slip angle (alpha) [rad]
      tire_msg.long_slip = tire->IF.Slp;         // Longitudinal slip (kappa) [-]
      tire_msg.turn_slip = tire->IF.TurnSlp;     // Turn slip [1/m]
      
      // Kinematic Parameters
      tire_msg.inclination_angle = tire->IF.InclinAngle;    // Camber angle [rad]
      tire_msg.effective_radius = tire->IF.rBelt_eff;       // Effective belt radius [m]
      tire_msg.belt_velocity = tire->IF.vBelt;              // Belt velocity [m/s]
      
      // Contact Point Information (global frame)
      tire_msg.contact_point_x = tire->IF.P_0[0];  // Contact point X [m]
      tire_msg.contact_point_y = tire->IF.P_0[1];  // Contact point Y [m]
      tire_msg.contact_point_z = tire->IF.P_0[2];  // Contact point Z [m]
      
      // Road Properties
      tire_msg.mu_road = tire->IF.muRoad;     // Road friction coefficient
      tire_msg.on_road = (tire->IF.onRoad != 0);  // Is tire on road?
      
      // Additional Tire State
      tire_msg.rim_rotation_speed = tire->IF.Rim_rotv;     // Rim angular velocity [rad/s]
      tire_msg.angular_velocity = tire->IF.Rim_rotv;       // Tire angular velocity [rad/s]
      
    } else {
      // If tire data is not available, set all to zero
      tire_msg.tire_index = static_cast<uint8_t>(tire_idx);
      tire_msg.fx = 0.0;
      tire_msg.fy = 0.0;
      tire_msg.fz = 0.0;
      ...
      tire_msg.my = 0.0;
      tire_msg.mz = 0.0;
      tire_msg.slip_angle = 0.0;
      tire_msg.long_slip = 0.0;
      tire_msg.turn_slip = 0.0;
      tire_msg.inclination_angle = 0.0;
      tire_msg.effective_radius = 0.0;
      tire_msg.belt_velocity = 0.0;
      tire_msg.contact_point_x = 0.0;
      tire_msg.contact_point_y = 0.0;
      tire_msg.contact_point_z = 0.0;
      tire_msg.mu_road = 0.0;
      tire_msg.on_road = false;
      tire_msg.rim_rotation_speed = 0.0;
      tire_msg.angular_velocity = 0.0;
    }
  };
  
  // Fill data for all four tires
  // Tire indices: 0=FL, 1=FR, 2=RL, 3=RR
  fillTireData(msg.front_left, 0);   // Front Left
  fillTireData(msg.front_right, 1);  // Front Right
  fillTireData(msg.rear_left, 2);    // Rear Left
  fillTireData(msg.rear_right, 3);   // Rear Right
}
...
int CMNodeHelloCM::userTestrunStartAtBegin() {
  ...
  { /* set up ground truth odometry publisher + job */
    typedef CMJob::RosPublisher<nav_msgs::msg::Odometry> OdomPub;
    auto job = std::make_shared<OdomPub>(nhp_, "odom");
    job->setCycleTime(10);  // 100Hz update rate
    job->registerCallback(&CMNodeHelloCM::odomFillMsg, this);
    scheduler_.addJob(job);
  }

  { /* set up tire steering publisher + job */
    typedef CMJob::RosPublisher<hellocm_msgs::msg::TireSteering> TireSteerPub;
    auto job = std::make_shared<TireSteerPub>(nhp_, "tire_steering");
    job->setCycleTime(10);  // 100Hz update rate
    job->registerCallback(&CMNodeHelloCM::tireSteeringFillMsg, this);
    scheduler_.addJob(job);
  }

  { /* set up tire forces publisher + job */
    typedef CMJob::RosPublisher<hellocm_msgs::msg::TireForcesArray> TireForcesPub;
    auto job = std::make_shared<TireForcesPub>(nhp_, "tire_forces");
    job->setCycleTime(10);  // 100Hz update rate
    job->registerCallback(&CMNodeHelloCM::tireForcesFillMsg, this);
    scheduler_.addJob(job);
  }
...
int CMNodeHelloCM::userTestrunEnd() {
  ...
  scheduler_.deleteJob("tire_steering");
  scheduler_.deleteJob("tire_forces");
  return 1;
}
